...
ALTER TABLE `build`
 ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `UNIQUE` (`name`), ADD KEY `project_id_FOREIGN` (`project_id`);

ALTER TABLE `e2e_test`
 ADD PRIMARY KEY (`id`);

ALTER TABLE `project`
 ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `git_url_UNIQUE` (`git_url`), ADD UNIQUE KEY `name_UNIQUE` (`name`);

ALTER TABLE `sprint`
 ADD PRIMARY KEY (`id`), ADD KEY `project_id_FOREIGN` (`project_id`);

ALTER TABLE `task`
 ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `UNIQUE` (`developer_id`,`sprint_id`), ADD KEY `sprint_id_FOREIGN` (`sprint_id`), ADD KEY `user_id_FOREIGN` (`developer_id`);

ALTER TABLE `user`
 ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `UNIQUE` (`name`);

ALTER TABLE `user_has_project`
 ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `UNIQUE` (`user_id`,`user_role`,`project_id`), ADD KEY `user_id_FOREIGN` (`user_id`), ADD KEY `project_id_FOREIGN` (`project_id`);

ALTER TABLE `user_story`
 ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `e2e_test_id_FOREIGN` (`e2e_test_id`), ADD KEY `project_id_FOREIGN` (`project_id`);

ALTER TABLE `user_story_has_build`
 ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `UNIQUE` (`user_story_id`,`build_id`), ADD KEY `build_id_FOREIGN` (`build_id`), ADD KEY `user_story_id_FOREIGN` (`user_story_id`);
...
ALTER TABLE `user_story`
ADD CONSTRAINT `user_story_fk_e2e_test` FOREIGN KEY (`e2e_test_id`) REFERENCES `e2e_test` (`id`) ON DELETE CASCADE,
